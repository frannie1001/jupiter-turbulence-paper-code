...
class PosData():
    ...
    def sys_3(self):
        for year in ['2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023', '2024']:
               ...
               spice.furnsh(f'/data/juno_spacecraft/data/new_kernels_and_data/mk/juno_{year}.tm')
            
        et_array = [spice.utc2et(i) for i in self.datetime_series.strftime('%Y-%m-%dT%H:%M:%S')]
        #positions, lt = spice.spkpos('JUNO', et_array,'IAU_JUPITER', 'NONE', 'JUPITER')
        positions, lt = spice.spkpos('JUNO', et_array,                         'JUNO_MAG_VIP4', 'NONE', 'JUPITER')
        #positions, lt = spice.spkpos('JUNO', et_array,'JUNO_JSS', 'NONE', 'JUPITER')
        rad = np.array([])
        lat = np.array([])
        lon = np.array([])
...
class bc_ids:
    ...
    def get_mp_bc(self):
        bc_df = pd.read_csv(f'/data/juno_spacecraft/data/crossings/wholecross5.csv')
    ...
    def sys_3_data(self):
        ...
        #positions, lt = spice.spkpos('JUNO', et_array,'IAU_JUPITER', 'NONE', 'JUPITER') # rotating frame
        positions, lt = spice.spkpos('JUNO', et_array,'JUNO_MAG_VIP4', 'NONE', 'JUPITER') # magnetic frame
        #positions, lt = spice.spkpos('JUNO', et_array, 'JUNO_JSS', 'NONE', 'JUPITER')
        rad = np.array([])
        lat = np.array([])
        lon = np.array([])
...
class WavData():
    ...
    def _get_data(self):
        ...
        del csv_df
        return [self.t, self.freq, self.data_df]
...
class MagClass(bc_ids):
    def __init__(self, timeStart, timeEnd):
        ...
        self.z_mag = 0.0
        self.bc_id = 0.0
        self.b = MagData(self.timeStart,self.timeEnd, '/data/juno_spacecraft/data/fgm_ss',['fgm_jno','r1s'])
        #self.b = MagData(self.timeStart,self.timeEnd, '/data/juno_spacecraft/data/juno_fgm_ss/fgm_ss',['fgm_jno','r60s'])
        self.read_mag_data()
        X,Y,Z = self.sys_3_data()
        self.bc_df = self.get_mp_bc()
        self.get_bc_mask()
    ...
    def read_mag_data(self):
        ...
        #b = MagData(self.timeStart,self.timeEnd,'/data/juno_spacecraft/data/fgm_ss',['fgm_jno','r1s'])
        #b = MagData(self.timeStart,self.timeEnd,'/data/juno_spacecraft/data/pickled_mag_pos',['jno_mag_pos','v01'])    
        
        bx = self.b.data_df['BX'][self.timeStart:self.timeEnd]
        by = self.b.data_df['BY'][self.timeStart:self.timeEnd]
        bz = self.b.data_df['BZ'][self.timeStart:self.timeEnd]
        x = self.b.data_df['X'][self.timeStart:self.timeEnd]
        y = self.b.data_df['Y'][self.timeStart:self.timeEnd]
        z = self.b.data_df['Z'][self.timeStart:self.timeEnd]
        
        self.t = bx.index
        #self.X,self.Y,self.Z = self.sys_3_data()
        r = np.sqrt(x**2 + y**2 + z**2)
        self.lat = myatan2(z,r)*180/np.pi
        #r = np.sqrt(self.X**2 + self.Y**2 + self.Z**2)
        #self.lat = myatan2(self.Z,r)*180/np.pi
...
class MagData():
    ...
    def _get_data(self):
        for mag_csv in self.data_files:
            ...
            csv_df = csv_df.set_index('DATETIME')
            csv_df.index = csv_df.index.astype('datetime64[ns]').floor('S')
            self.data_df = pd.concat([self.data_df,(csv_df)])
            self.data_df = self.data_df.sort_index()
            self.data_df = self.data_df[self.start_time: self.end_time].sort_index()
        return self.data_df
        #del csv_df
...
class Turbulence(MagData):
    def __init__(self, start_iso, end_iso, dt, interval, window_size=10,
    ...
    def _get_q(self, data_df):
        ...
        num_intervals = np.ceil((data_df.index.max().to_pydatetime()
                         - data_df.index.min().to_pydatetime())/(timedelta(minutes=self.interval)))
        for i in range(int(num_intervals)):
            ...
            interval_time_series = data_df[start_index: end_index].index.to_pydatetime()
           # if len(interval_time_series) < 30: #changed from seconds (1800) to minutes (30), might need to change back once everything is 1 sec data
            #  continue
            interval_bx = data_df.BX[start_index: end_index].to_numpy()
            interval_by = data_df.BY[start_index: end_index].to_numpy()
            interval_bz = data_df.BZ[start_index: end_index].to_numpy()
            interval_perp1 = data_df.B_PERP1[start_index: end_index].to_numpy()
            interval_perp2 = data_df.B_PERP2[start_index: end_index].to_numpy()
...
class JadClass(bc_ids):
    ...
    def get_ion_data(self):      
        ...
        print('jad_arr...',np.shape(self.jad_arr))
        self.energy_scale = np.array(jadeIon.ion_dims)/1000
        return [self.t, self.energy_scale, self.jad_arr]
    ...
    def get_elec_data(self):      
        ...
        return [self.t, self.energy_scale_elec, self.jad_e_arr]
